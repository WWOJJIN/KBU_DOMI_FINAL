...
def restore_room_structure():
    ...
    try:
        ...
        for room_num in range(601, 647):
            rooms_to_create.append((building, f"{room_num}호", 1, "1인실"))
        
        # 7층: 2인실 (701-746호)
        for room_num in range(701, 747):
            rooms_to_create.append((building, f"{room_num}호", 2, "2인실"))
        
        # 8층: 3인실 (801-828호)
        for room_num in range(801, 829):
            rooms_to_create.append((building, f"{room_num}호", 3, "3인실"))
        
        # 9층: 룸메이트 신청자용 (901-920호)
        for room_num in range(901, 921):
            rooms_to_create.append((building, f"{room_num}호", 2, "룸메이트"))
        
        # 10층: 방학이용 (1001-1020호)
        for room_num in range(1001, 1021):
            rooms_to_create.append((building, f"{room_num}호", 1, "방학이용"))
        ...
        insert_query = """
        INSERT INTO Room_Info (building, room_number, max_occupancy, room_type, current_occupancy, status, floor_number)
        ...
        cursor.execute("""
            SELECT building, 
                   CASE 
                       WHEN room_number LIKE '6%' THEN '6층(1인실)'
                       WHEN room_number LIKE '7%' THEN '7층(2인실)'
                       WHEN room_number LIKE '8%' THEN '8층(3인실)'
                       WHEN room_number LIKE '9%' THEN '9층(룸메이트)'
                       WHEN room_number LIKE '10%' THEN '10층(방학이용)'
                   END as floor_type,
                   COUNT(*) as count
            FROM Room_Info 
            GROUP BY building, floor_type
            ORDER BY building, floor_type
        """)
